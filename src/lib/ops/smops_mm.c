...
void mult_to_dense(MATRIX_DATA *dense_matrix, MATRIX_DATA a, MATRIX_DATA b, TYPE type, int pos)
{
    switch(type) {
        case FLOAT:
            #pragma omp atomic
                        ...
                        for(int i = p_a; i < q_a; i++) {
                            for(int j = p_b; j < q_b; j++) {
                                if(csr_a->ja[i] == csc_b->ja[j]) {
                                    mult_to_dense(dense_matrix, csr_a->nnz[i], csc_b->nnz[j], type, index);
                                    break;
                                }
                            }
                        }
